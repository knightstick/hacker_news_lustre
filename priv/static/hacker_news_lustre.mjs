...
function await_list(xs) {
  let _pipe = xs;
  let _pipe$1 = all_promises(_pipe);
  return map_promise(_pipe$1, toList);
}
...
// build/dev/javascript/hacker_news_lustre/hacker_news_lustre.mjs
var State = class extends CustomType {
  constructor(story_items) {
    super();
    this.story_items = story_items;
  }
};
var HackerNewsItem = class extends CustomType {
...
var HasNotStartedYet = class extends CustomType {
};
var InProgress = class extends CustomType {
};
var Resolved = class extends CustomType {
  constructor(x0) {
    super();
    this[0] = x0;
  }
};
var Started = class extends CustomType {
};
...
var LoadStoryItems = class extends CustomType {
  constructor(x0) {
    super();
    this[0] = x0;
  }
...
function init2(_) {
  let initial_state = new State(new HasNotStartedYet());
  let initial_effect = from2(
    (dispatch) => {
      return dispatch(new LoadStoryItems(new Started()));
    }
  );
  return [initial_state, initial_effect];
}
    ...
    toList([
      (() => {
        ...
        if ($ instanceof Some) {
          let url = $[0];
          return a(
            toList([src(url), target("_blank")]),
            toList([text(item.title)])
          );
        } else {
          return p(toList([]), toList([text2(item.title)]));
        }
      })()
    ])
  );
...
function spinner() {
  return div(
    toList([style(toList([["textAlign", "center"], ["marginTop", "15"]]))]),
    toList([text("Loading...")])
  );
}
function render_error(error_message) {
  return h1(
    ...
    toList([text(error_message)])
  );
}
function render_items(story_items) {
  if (story_items instanceof HasNotStartedYet) {
    return div(toList([]), toList([]));
  } else if (story_items instanceof InProgress) {
    return spinner();
  } else if (story_items instanceof Resolved && !story_items[0].isOk()) {
    let err = story_items[0][0];
    return render_error(err);
  } else {
    let items = story_items[0][0];
    return div(toList([]), map3(items, render_item));
  }
}
function view(state) {
  return div(
    ...
    toList([
      h1(toList([]), toList([text("Lustre Hackernews")])),
      render_items(state.story_items)
    ])
  );
}
function parse_ids(body) {
  let decoder = list(int);
  let _pipe = decode_bits(body, decoder);
...
function parse_item(body) {
  let decoder = decode3(
    (var0, var1, var2) => {
      return new HackerNewsItem(var0, var1, var2);
    },
    field("id", int),
    field("title", string),
    optional_field("url", string)
  );
  return decode_bits(body, decoder);
}
function load_story_item(id) {
  let endpoint = "https://hacker-news.firebaseio.com/v0/item/" + to_string2(
    id
  ) + ".json";
  let $ = to(endpoint);
  if (!$.isOk()) {
  ...
  let req = $[0];
  let _pipe = send(req);
  let _pipe$1 = try_await(_pipe, read_bytes_body);
  let _pipe$2 = map_promise(
    _pipe$1,
    (_capture) => {
      return map_error(_capture, (_) => {
        return "Fetch Error";
      });
    }
  );
  return try_await(
    ...
    (resp) => {
      let body = resp.body;
      let _pipe$3 = body;
      let _pipe$4 = parse_item(_pipe$3);
      let _pipe$5 = map_error(_pipe$4, (_) => {
        return "Parse Error";
      });
      return resolve(_pipe$5);
    }
  );
}
function load_items_from_ids(ids) {
  let _pipe = ids;
  let _pipe$1 = take(_pipe, 10);
  let _pipe$2 = map3(_pipe$1, load_story_item);
  let _pipe$3 = await_list(_pipe$2);
  return map_promise(_pipe$3, (list2) => {
    return all(list2);
  });
}
var stories_endpoint = "https://hacker-news.firebaseio.com/v0/topstories.json";
function load_item_ids() {
  let $ = to(stories_endpoint);
  if (!$.isOk()) {
    throw makeError(
      "assignment_no_match",
      "hacker_news_lustre",
  ...
  return try_await(
    ...
    (resp) => {
      let body = resp.body;
      return resolve(parse_ids(body));
    }
  );
}
function load_story_items() {
  return from2(
    (dispatch) => {
      let _pipe = load_item_ids();
      let _pipe$1 = try_await(_pipe, load_items_from_ids);
      let _pipe$2 = map_promise(
        _pipe$1,
        (result) => {
          return new LoadStoryItems(new Finished(result));
        }
      );
      tap(_pipe$2, (message) => {
...
function update2(_, msg) {
  if (msg instanceof LoadStoryItems && msg[0] instanceof Started) {
    let next_state = new State(new InProgress());
    return [next_state, load_story_items()];
  } else if (msg instanceof LoadStoryItems && msg[0] instanceof Finished && msg[0][0].isOk()) {
    let story_items = msg[0][0][0];
    let next_state = new State(new Resolved(new Ok(story_items)));
    return [next_state, none()];
  } else {
    let err = msg[0][0][0];
    let next_state = new State(new Resolved(new Error(err)));
    return [next_state, none()];
  }
}
function main() {
